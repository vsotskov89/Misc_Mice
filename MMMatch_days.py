...
path = 'D:\\Work\\NEW_MMMICE\\'
path1 = 'D:\\Work\\PLACE_CELLS\\MIN1PIPE\\'

names = ['CA1_23', 'CA1_24', 'CA1_25', 'CA1_22', 'NC_701', 'NC_702', 'G7F_2', 'FG_2', 'NC_761'] #'G7F_1', 'G6F_01', 'FG_1']
names2 = names[0:-1]
names3 = names[0:3]
...
p_names = ['CA1_22', 'CA1_25', 'NC_701']
r_names = ['CA1_23', 'CA1_24', 'NC_702', 'G7F_2', 'FG_2']

nik_names = ['CA1_22', 'CA1_23', 'CA1_24', 'CA1_25', 'NC_702', 'NC_701']

fast_names = ['G7F_1_1D','G7F_1_2D','G7F_2_1D','G7F_2_2D','G6F_01_1D']
...
def NSpecTransitionMatrix(ms1, ms2, match_fname, d1, d2):
    max_n_fields = 34
    CF = np.zeros((max_n_fields+1, max_n_fields+1))
    match = np.genfromtxt(match_fname, delimiter=',')
    for row in range(np.size(match,0)):
        pf1 = FindPlaceField(ms1, match[row,d1] - 1)        
        pf2 = FindPlaceField(ms2, match[row,d2] - 1) 
        ...
        if pf1[0].mu >= 361:
            ...
            n1 = min([pf.n_spec for pf in pf1])  
            
        if pf2[0].mu >= 361:
            n2 = max_n_fields
        else:
            n2 = min([pf.n_spec for pf in pf2])
...
#%% Reconstruction of G67 mice from fast spikes
min_sel_len = 10

for name in ['G7F_2_2D']:

    ms = pio.LoadMouse(path + name + '_nnew.npz')
    ms.get_spike_data(path + 'fast spikes\\spikes_' + name + '_NR_data_processed_traces.csv')
    ...
    np.savez(path + name + '_nnew.npz', [ms])
...
#%% Draw flat place fields

try:
    os.mkdir(path + 'flat_plots\\')
except:
    pass
...
for name in names:
    for d in [1]:
        try:
            ...
            continue        
        drcl.DrawFlatPlaceFields(ms, outpath = path + 'flat_plots\\' + name + '_' + str(d))
...
for name in names:
    for d in [1,2,3]:
        ...
        print(name + '_' + str(d) + 'D\t' + str(ms.spec_score[4])+ '\t' + str(max(ms.spec_score)) + '\t' + str(n_inst))
...
#   days = [1,2,3]
    ...
    for i, d in enumerate(days):
        ...
        if i == 0:
            ...
            nums = TransmitNums(path + name + '_match.csv', days[0], d, nums0)
        ...
        for i, n_spec in enumerate(n_spec_list):    
            ax.add_patch(patches.Polygon([[n_spec+0.2, i*fact],[n_spec+0.2, (i+1)*fact],[n_spec+1, (i+0.5)*fact]], facecolor = 'red', edgecolor = 'none'))
        ax.imshow(sel_score_map, extent = (0, max_n_visits, max_n_visits*aspect_r, 0))
    plt.savefig(path + name + '_matched_sel_score_reverse3_40.png') 
...
for d in [1,2,3]:
    ...
    # r_sscore = []
    for name in names:
        if name == 'CA1_22' and d==3 or name == 'G7F_1' and d==2:
            continue
        try:
            ms = pio.LoadMouse(path + name + '_' + str(d) + 'D_nnew.npz')
...
for name in names3:
    ...
    plt.figure()
    TR = NSpecTransitionMatrix(ms[0], ms[2], path + name + '_match.csv', 0, 2) 
    TR[TR>0]+=2
    plt.imshow(TR, cmap = 'jet')
    # for (j,i),label in np.ndenumerate(TR):
    #     plt.text(i,j,int(label),ha='center',va='center', size = 'xx-large')
    plt.savefig(path + name + '_nspec_matrix_13.png')    
    
    plt.figure() 
    TR = NSpecTransitionMatrix(ms[1], ms[2], path + name + '_match.csv', 1, 2)
    TR[TR>0]+=2
    plt.imshow(TR, cmap = 'jet')
    # for (j,i),label in np.ndenumerate(TR):
    #     plt.text(i,j,int(label),ha='center',va='center', size = 'xx-large')
    plt.savefig(path + name + '_nspec_matrix_23.png')     
