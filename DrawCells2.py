    ...
    if polar:  
        ...
        if mode == "None":
            ...
            for n_bin in range(Nbins):
                ...
                ax.plot(Mouse.angle[Mouse.neuro_bin[:,cell] == n_bin]*np.pi/180, Mouse.time[Mouse.neuro_bin[:,cell] == n_bin], c = color, ls = ' ', ms=2, marker='.', zorder = 0.0) 
...
def DrawAllSuper(Mouse, k_word):
   for i, cell in enumerate(Mouse.super_cells):
       plt.clf()
       fig = plt.figure(figsize=(10,10), dpi=300)
       ax = fig.add_subplot(111, projection='polar') 
       fig.suptitle(Mouse.name + ' cell_' + str(cell + 1) + ' MI =' + str(Mouse.mi_score[cell]), fontsize=16)
       ax = DrawField(Mouse.super_mu[i], Mouse.super_std[i], Mouse.time[0], Mouse.time[len(Mouse.angle)-1], ax = ax)
       plot(Mouse, cell, mode='Neuro_bin', k_word = k_word, polar=True, save=False, ax = ax, line_width = 1.5)
       plt.savefig(k_word + '_' + str(i) + '_' + str(cell + 1) + '_circ.png')
       plt.close() 
...
def DrawAllTrue(Mouse, k_word):
   for i in range(len(Mouse.true_cells)):
       ...
       fig = plt.figure(figsize=(10,10), dpi=300)
       ax = fig.add_subplot(111, projection='polar') 
       fig.suptitle(Mouse.name + ' cell_' + str(Mouse.true_cells[i]+1) + ' MI =' + str(Mouse.mi_score[i]), fontsize=16)
       ax = DrawField(Mouse.mu_list[i], Mouse.std_list[i], Mouse.time[0], Mouse.time[len(Mouse.angle)-1], ax = ax)
       plot(Mouse, Mouse.true_cells[i], mode='Neuro_bin', k_word = k_word, polar=True, save=False, ax = ax, line_width = 1.5)
       plt.savefig(k_word + '_' + str(i) + '_' + str(Mouse.true_cells[i]+1) + '_circ.png')
       plt.close()  
...
def DrawAllCells(Mouse, k_word):
   ...
   for i in range(len(Mouse.spikes[0])): #first draw all usual cells
       ...
       fig = plt.figure(figsize=(10,10), dpi=300)
       ax = fig.add_subplot(111, projection='polar') 
       fig.suptitle(Mouse.name + ' cell_' + str(i+1) + ' KS_stat =' + str(Mouse.ks_stat[i]) + ' KS_p_score =' + str(Mouse.ks_p_score[i]) + ' ' + str(i in Mouse.super_cells), fontsize=16)
       plot(Mouse, i, mode='Neuro_bin', k_word = k_word, polar=True, save=False, ax = ax, line_width = 1.5)
       plt.savefig(k_word + '_' + str(i+1) + '_circ.png')
       plt.close()  
   ...
   for i, cell in enumerate(Mouse.true_cells):
       ...
       fig = plt.figure(figsize=(10,10), dpi=300)
       ax = fig.add_subplot(111, projection='polar') 
       fig.suptitle(Mouse.name + ' cell_' + str(cell+1) + ' KS_stat =' + str(Mouse.ks_stat[cell]) + ' KS_p_score =' + str(Mouse.ks_p_score[cell]) + ' ' + str(cell in Mouse.super_cells), fontsize=16)
       
       if i in Mouse.super_in_true:
           for j,ts in enumerate (Mouse.t_spec[supcl]):
               ax = DrawField(Mouse.mu_list[i], Mouse.std_list[i], ts/20, Mouse.times_out[i][Mouse.n_spec[supcl][j] + Mouse.spec_len[supcl][j] - 1]/20, ax = ax) #Mouse.times_out[i][Mouse.spec_len[supcl][j] - 1]/20
           supcl += 1
       else:
           ax = DrawField(Mouse.mu_list[i], Mouse.std_list[i], Mouse.time[0], Mouse.time[-1], ax = ax, color = 'c')
        
       plot(Mouse, cell, mode='Neuro_bin', k_word = k_word, polar=True, save=False, ax = ax, line_width = 1.5)
       plt.savefig(k_word + '_' + str(cell+1) + '_true_' + str(i+1) + '_circ.png')
       plt.close()         
...
def DrawNeuroPasses(passes, k_word):
    for d,day_p in enumerate(passes):
#       sort_ind = np.sort(mu_list[d])
       plt.clf()
       fig = plt.figure(figsize=(10,10), dpi=300) 
       fig.suptitle('Day '+ str(d+1) + ' ' + k_word, fontsize=16)
       ax = fig.add_subplot(111)
       bmap = np.zeros((int(len(day_p)/10), 10))
       for i in range(int(len(day_p)/10)):
           bmap[i,:] = np.array(day_p[int(i)*10:(int(i)+1)*10])
#           bmap[i,:] = np.array(day_p[int(sort_ind[i])*10:(int(sort_ind[i])+1)*10])
#           bmap[i,:] = (bmap[i,:]-np.min(bmap[i,:]))/(np.max(bmap[i,:])-np.min(bmap[i,:]))
       ax.imshow(bmap, cmap='jet',interpolation = 'nearest')
       plt.savefig(k_word + '_'+ str(d+1) + 'D_map.png')       
       
       plt.clf()
       fig = plt.figure(figsize=(10,10), dpi=300)
       ax = fig.add_subplot(111)
       fig.suptitle('Day '+ str(d+1) + ' ' + k_word, fontsize=16)
       ax.plot(np.mean(bmap, axis = 0))
       plt.savefig(k_word + '_'+ str(d+1) + 'D_average.png')       
...
def DrawPlaceCells(Mouse, rad, outname):
       dt  = 5
       tlen = Mouse.time[-1]
       ...
       fig = plt.figure(figsize=(10,10), dpi=300)
       ax = fig.add_subplot(111, projection='polar') 
       fig.suptitle(Mouse.name + 'place fields', fontsize=16)  
       for i,cell in enumerate(Mouse.super_in_true):
            DrawField(mu = Mouse.mu_list[cell], std = Mouse.std_list[cell], t_spec = Mouse.t_spec[i][0]*rad/tlen, t_end = (Mouse.t_spec[i][0]+dt)*rad/tlen, ax=ax, color = colornum_Metro(Mouse.true_cells[cell]+1))
...
def DrawContourMap(Mouse, filterpath, f_out):
    #coloring cell contours with respect to their place fields
    ...
    for cell,f in enumerate(fnames):
        ...
        if cell in pcs: #place cells, including bimodal
            ...
            for ipf, pf in enumerate(Mouse.pc[pcs.index(cell)].pf):
                color_in.append(cm.jet(pf.mu/180)[0:-1])
...
def DrawRegressionMap(f_in, f_out):
    im = np.genfromtxt(f_in, delimiter=' ', skip_header=0)
    im = np.transpose((im - np.min(im))/(np.max(im) - np.min(im)))
    io.imshow(im)
    io.imsave(f_out, im)


def DrawKScoreMap(Mouse, filterpath, f_out):
    #coloring cell contours with respect to their KS-score
    fnames = glob.glob(filterpath+'\\*.tif')
    im = io.imread(fnames[0])
...
def DrawXYMap(Mouse, cell, f_out, **kwargs):
...
def DrawTSpecVsMuDistrib(Mouse, f_out):
    ...
    for pc in Mouse.pc:
        for pf in pc.pf:
            mu_list.append(pc.pf.mu)
            t_spec_list.append(pc.pf.t_spec)
            
    plt.clf()
    fig = plt.figure(figsize=(10,10), dpi=300)
    fig.suptitle(Mouse.name + 'mu vs t_spec', fontsize=16)
           
    plt.scatter(mu_list,t_spec_list)
